...
def renderFrame(index):
  ...
  for (i, point) in enumerate(circlePoints):
    ...
    r, g, b= [255, 255, 255]
    # draw.line((, , , ), fill = (int(r * 255), int(g * 255), int(b * 255), 50))
    print(x + width / 2, -45, y + height / 2, targetX + width / 2, targetY + height / 2)
    d.append(draw.Lines(x + width / 2, y + height / 2, targetX + width / 2, targetY + height / 2, close=True, stroke=''))
  ...
  # d.save(f'images/mandelbrote-times-table{index:0>4d}.png')
  d.saveSvg(f'images/mandelbrote-itmes-table{index:0>4d}.svg')
